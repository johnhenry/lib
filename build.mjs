...
const wrapInTemplate = async (bodyHtml) => {
  if (!templateParts) {
    ...
    if (at === -1) throw new Error("templates/start.html: no <body></body> to splice into");
    templateParts = [
...
async function buildIndices(top, depth) {
  ...
  const items = children
    ...
    .join("");
  await writeGenerated(join(top, OUTFILE), await wrapInTemplate(`<ul>${items}</ul>`));
...
async function buildModule(section, name) {
  ...
  const versionDir = versions.length ? join(moduleDir, version) : `${moduleDir}/`;
...
async function reportPublishable(section, name) {
  ...
  try {
    ...
    if (pkg.publish === true && pkg.name === name && pkg.version === version) {
      console.log(`🚀 publishable: ${name}@${version} (${section}/${name}/${version})`);
    }
  } catch {
...
async function buildRootPages() {
  const mdToHtml = async (input, output) =>
    writeGenerated(
      output,
      await wrapInTemplate(renderMarkdown(await readFile(join(ROOT, input), "utf8")))
    );
