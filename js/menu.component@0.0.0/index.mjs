    ...
    switch (name) {
      case "pushed":
        ...
        if (current === null) {
          ...
          this.reset();
          this.dispatchEvent(
            new CustomEvent("popped", {
              bubbles: true,
              composed: true,
              detail: { pushed: old, clicked: this.lastClicked, node },
            })
          );
          return;
        }
        ...
        setTimeout(() => {
          ...
          if (!Number.isNaN(Number(current))) {
            element = [...this.kids][
              ((Number(current) % length) + length) % length
            ];
          } else {
          ...
          if (template) {
            ...
            this.content.addEventListener(element.dataset.endEvent, this.pop, {
              once: true,
            });
          }
          ...
          this.dispatchEvent(
            new CustomEvent("pushed", {
              bubbles: true,
              composed: true,
              detail: { pushed: current, clicked: this.lastClicked, node },
            })
          );
        });
    }
  ...
  reset() {
    ...
    for (const child of this.kids) {
      ...
      if (kid.getElementsByTagName("template")[0]) {
        ...
        kid.onclick = (event) => {
          event.stopPropagation();
          this.dispatchEvent(new Event(this.endEvent, { bubbles: true }));
        };
      }
      index++;
    }
