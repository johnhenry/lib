...
export const teeAsync = (num) => (asyncIterator) => {
  const iterators = [];
  // (1.0.0) `for (const [,] of iterateSync(num))` destructured the yielded
  // numbers and threw "number is not iterable"; teeAsync never worked before.
  for (const _ of iterateSync(num)) {
